...
class LearnedGroupConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.conv = ConvCust(in_channels, out_channels, stride=stride, 
        ...
        self.register_buffer('_mask', torch.ones(self.conv.chpref.size()))
    ...
    def _dropping(self, delta):
        weight = (self.conv.chpref+self.conv.bias) * self.mask
        ### Sum up all kernels
...
class ConvCust(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, kernel_size=3, padding=0, bias=True):
        ...
        if bias:
            self.bias = nn.Parameter(torch.Tensor(out_planes, 1, 1, 1))
        else:
