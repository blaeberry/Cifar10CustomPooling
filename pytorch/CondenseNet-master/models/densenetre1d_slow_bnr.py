...
class ConvCust(nn.Module):
    def __init__(self, in_planes, out_planes, stride=1, kernel_size=1, padding=0, 
        ...
        self.height = height
        self.bias = bias
        self.bnr1 = nn.Sequential(nn.BatchNorm2d(height),
                                  nn.ReLU(inplace=True))
        self.yw = nn.Parameter(torch.Tensor(int(height*stride), height, *self.kernel_size))
        self.bnr2 = nn.Sequential(nn.BatchNorm2d(width),
                                 nn.ReLU(inplace=True))
...
class DenseNetRe1dSlowBNR(nn.Module):
    def __init__(self, args):

        super(DenseNetRe1dSlowBNR, self).__init__()
