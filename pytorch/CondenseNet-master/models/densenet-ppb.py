...
class prefpoolb(nn.Module):
    def __init__(self, in_planes, out_planes, stride=2, kernel_size=2, padding=0, 
        ...
        self.maxpref = nn.Parameter(in_planes)
        self.pconvs = nn.Parameter(1, 1, *self.kernel_size, num_convs)
        self.pprefs = nn.Parameter(in_planes, num_convs)
        if bias:
    ...
    def forward(self, x):
        out = (mb+maxpref)*self.maxpool(x)
        for c in self.nc:
            pconv = self.pconvs.select(4, c)
            pconv = pconv.repeat(self.out_channels,1,1,1)
            ppref = self.pb.select(0, c) + self.ppref.select(1, c)
            pconv = (ppref)*pconv
            out += F.conv2d(x, pconv, None, stride, padding, groups = self.in_channels)
        return out
