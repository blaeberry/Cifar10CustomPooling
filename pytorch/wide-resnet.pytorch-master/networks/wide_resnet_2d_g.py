...
class wide_basic(nn.Module):
    def __init__(self, in_planes, planes, dropout_rate, groups, stride=1):
        ...
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(
                ConvCust(in_planes, planes, groups=1, kernel_size=1, stride=stride, bias=True),
            )
...
class Wide_ResNet_2D_G(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, groups):
        super(Wide_ResNet_2D_G, self).__init__()
        self.in_planes = 16
...
if __name__ == '__main__':
    net=Wide_ResNet_2D_G(28, 10, 0.3, 10)
