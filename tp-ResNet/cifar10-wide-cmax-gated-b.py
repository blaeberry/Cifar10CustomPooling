...
def custom_pooling2d(inputs, var_scope, padding, nf = 4, strides = [1, 1, 2, 2], data_format='NCHW'):
    with tf.variable_scope(var_scope):
        ...
        mb = tf.get_variable("max_weights", (1), initializer=tf.constant_initializer(0.5))
        for k in range(nf):
            ...
            p = p + (pw+pb)*tf.nn.depthwise_conv2d(l, pcon, strides, padding, data_format=data_format, name='depthwise{}'.format(k))
        p = tf.add((mw+mb)*max_inputs, p, name = "outputs")    
    return p
