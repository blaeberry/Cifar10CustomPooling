...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        def expand_old_filters(filters, in_channels):
            num_filters = filters.get_shape().as_list()[2]
            with tf.variable_scope('old_filter'):
                ch_prefs = tf.get_variable('ch_prefs', (in_channels, num_filters), 
                    initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out'))
            kernel = tf.expand_dims(filters, axis = 2) #w,h,1,num_filters
            kernel = kernel * ch_prefs #w,h,1,num_filters * in_channels,num_filters
        ...
        def residual(name, l, filters1, filters2, increase_dim=False, first=False):
            ...
            with tf.variable_scope(name):
                ...
                if not first:
                    kernel = expand_old_filters(filters1, in_channel)
                    with tf.variable_scope('old1'):
                ...
                with tf.variable_scope('second'):
                    kernel, filters2 = expand_filters(out_channel, out_channel)
                ...
                if increase_dim:
                    ...
                    d2 = tf.pad(d2, [[0, 0], [in_channel // 2, in_channel // 2], [0, 0], [0, 0]])

                l = c2 + l + d2
                return l, filters1, filters2
        ...
        with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='NCHW'), \
            ...
            for k in range(1, self.n):
                l, filters1, filters2 = residual('res1.{}'.format(k), l, filters1, filters2)
            # 32,c=16

            l, filters1, filters2 = residual('res2.0', l, filters1, filters2, increase_dim=True)
            for k in range(1, self.n):
                l, filters1, filters2 = residual('res2.{}'.format(k), l, filters1, filters2)
            # 16,c=32

            l, filters1, filters2 = residual('res3.0', l, filters1, filters2, increase_dim=True)
            for k in range(1, self.n):
                l, filters1, filters2 = residual('res3.' + str(k), l, filters1, filters2)
            l = BNReLU('bnlast', l)
            # 8,c=64
