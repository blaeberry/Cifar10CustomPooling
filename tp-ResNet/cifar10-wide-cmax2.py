...
def custom_pooling2d(inputs, var_scope, padding, strides = [1, 2, 2], data_format='NCHW'):
    ...
    convolves = tf.add(tf.add(pw0*tf.nn.convolution(cinputs, pcon0, 'VALID', strides = strides),
        pw1*tf.nn.convolution(cinputs, pcon1, 'VALID', strides = strides)), pw2*tf.add(tf.nn.convolution(cinputs, pcon2, 'VALID', strides = strides),
