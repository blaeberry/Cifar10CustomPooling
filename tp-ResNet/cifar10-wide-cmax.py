...
def custom_pooling2d(inputs, var_scope, padding, strides = [2, 2, 1], data_format='NCHW'):
    ...
    convolves = tf.nn.convolution(cinputs, pcon0, 'SAME', strides = strides, data_format = data_format) + 
        tf.nn.convolution(cinputs, pcon1, 'SAME', strides = strides, data_format = data_format) + 
        tf.nn.convolution(cinputs, pcon2, 'SAME', strides = strides, data_format = data_format) + 
        tf.nn.convolution(cinputs, pcon3, 'SAME', strides = strides, data_format = data_format)
    print(convolves.get_shape())
    convolves = tf.squeeze(convolves)
    return tf.multiply(max_inputs, max_weight) + convolves
