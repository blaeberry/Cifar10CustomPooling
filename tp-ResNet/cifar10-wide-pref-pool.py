...
def pref_pooling(inputs, var_scope, padding, strides = [1, 2, 2], data_format='NCHW'):
    ...
    with tf.variable_scope(var_scope):
        max_weight = tf.get_variable("max_weights", (in_channels, 1, 1, 1), initializer=tf.constant_initializer(0.4))
    ...
    convolves = tf.add(
        tf.add(
            tf.nn.convolution(tf.multiply(pw0, cinputs), pcon0, 'VALID', strides = strides),
            tf.nn.convolution(tf.multiply(pw1, cinputs), pcon1, 'VALID', strides = strides)), 
        tf.add(
            tf.nn.convolution(tf.multiply(pw2, cinputs), pcon2, 'VALID', strides = strides),
            tf.nn.convolution(tf.multiply(pw3, cinputs), pcon3, 'VALID', strides = strides)))
