    ...
    with tf.variable_scope(var_scope):
        ...
        for k in range(nf):
            ...
            pcon = tf.get_variable('pcon{}'.format(k), weights_shape, 
                initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')) + pw
            pcon = tf.tile(pcon, [1, 1, in_shape[1], 1], name='tiled')
            p = p + tf.nn.depthwise_conv2d(l, pcon, strides, padding, data_format=data_format, name='depthwise')
