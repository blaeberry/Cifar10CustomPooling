...
def custom_conv(inputs, pcon):
    return tf.nn.convolution(inputs, pcon, 'VALID')
    #should do 1x1 conv2d here instead
...
def custom_pooling2d(name, inputs, nf = 4):
    with tf.variable_scope(name):
        ...
        pidx0 = np.arange(in_channel) # [0:(in_channel-1)]
        pidx = np.repeat(pidx0, 4) #[0 0 0 0 1 1 1 1 2 ... in_channel-1]
        #it naturally prepends the dims before tiling
        pidx = np.tile(pidx, [tf.shape(l)[0], int(in_shape[1] // 2), int(in_shape[2] // 2), pidx.size])
        parts = tf.dynamic_partition(patches, pidx, in_channel)
        ...
        for k in range(nf):
            ...
            conv = tf.map_fn(custom_conv, parts)
            conv = tf.dynamic_stitch(pidx0, conv)
            p = p + pw*conv
