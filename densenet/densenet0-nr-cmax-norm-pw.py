...
def custom_pooling2d(name, inputs, nf = 4, strides = [2, 2, 1]):
    with tf.variable_scope(name):
        ...
        pw = tf.nn.relu(tf.get_variable("pw", (nf+1), initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out')))
        pw = pw / tf.reduce_sum(pw)
        for k in range(nf):
            pcon = tf.get_variable('pcon{}'.format(k), weights_shape, 
                initializer=tf.variance_scaling_initializer(scale=2.0, mode='fan_out'))
            p = p + (pw[k])*tf.nn.convolution(cinputs, pcon, 'VALID', strides = strides)
        p = tf.squeeze(p, axis=-1, name = 'convolves')
        p = tf.add((pw[nf])*max_inputs, p, name = "outputs")    
    return p
